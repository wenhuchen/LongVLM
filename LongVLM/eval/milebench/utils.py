...
class MileBenchDataset(Dataset):
    ...
    def __getitem__(self, index):
...

        {
            ...
            'context': 'what is the image <image> about?',
            'response': '',
        }
        '''
        ...
        if self.combine_image:
            ...
            for i in range(img_num):
                rmv_txt = '{image#%d}'% (i+1)
                rmv_tbl = '{table#%d}'% (i+1)
                context = context.replace(rmv_txt, f'<image> ')
                context = context.replace(rmv_tbl, f'<image> ')
            # context is without instruction here!
            # context = '<image>'*self.combine_image + '\n' + context # we do this later
        else:
            for i in range(img_num):
                rmv_txt = '{image#%d}'% (i+1)
                rmv_tbl = '{table#%d}'% (i+1)
        ...
        if 'sub_meta_data' in ann:
            sub_meta_data = ann['sub_meta_data']
            context_length = sub_meta_data['context_length']
            context_length_text = sub_meta_data['context_length_text']
            num_images = sub_meta_data['num_images']
            ...
            return {
                ...
                "context_length_text": context_length_text,
    ...
    def collate_fn(self, batch):
        ...
        # batch_data['context_length'] = [sample['context_length'] for sample in batch]
        # batch_data['context_length_text'] = [sample['context_length_text'] for sample in batch]
        # batch_data['num_images'] = [sample['num_images'] for sample in batch]
        return batch_data
