...
def param_classification(name):
    ...
    if name in ['query_tokens', 'text_projection', 'logit_scale']:
        ...
        return 'layer_scale'
    elif 'pooling_layer' in name:
        return 'layer_scale'
    elif 'local_posid' in name:
        return 'layer_scale'
    else:
        return 'other'
...
def create_optimizer(self):
    ...
    try:  # for stage2 model
        ...
        qllama_num_layers = opt_model.config.llm_config.num_hidden_layers + 2
    except:  # for stage3 model
        vit_num_layers = opt_model.internvl.config.vision_config.num_hidden_layers + 2
        qllama_num_layers = opt_model.internvl.config.llm_config.num_hidden_layers + 2
    ...
    for name, param in opt_model.named_parameters():
        ...
        if len(param.shape) == 1 or name.endswith('.bias') or 'layer_scale' in name:
            group_name = 'no_decay'
            this_weight_decay = 0.
        else:
            group_name = 'decay'
