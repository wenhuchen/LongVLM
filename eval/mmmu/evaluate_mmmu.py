    ...
    if args.rope_pos_id_version !='default':
        ...
        def __getitem__(self, idx):
            ...
            if self.dynamic_image_size:
                ...
                orig_sizes.append(pil_images[0].size)
                image_list.append([pil_images[0]])
                num_tiles.append(1)
            ...
            prefix_str_list = []
            for i in range(record_num_image):
                rmv_txt = '<image %d>' % (i + 1)
                prefix_str = f'The {rmv_txt} is <image>'
                prefix_str_list.append(prefix_str)
            question = ', '.join(prefix_str_list) + '\n' + question
