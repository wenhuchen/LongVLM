...
def main(args):
    ...
    json.dump(
        result, 
        open(os.path.join(result_dir,  'eval_milebench', 'milebench_result.json'), 'w'), 
        ensure_ascii=False, indent=4)
    print(f"write result in {os.path.join(result_dir,  'eval_milebench', 'milebench_result.json')}")
    ...
    df.to_csv(os.path.join(result_dir, 'eval_milebench',  'milebench_result.csv'), index=False)
...
python eval/milebench/score.py \
    --result-dir work_dirs/share_internvl/InternVL2-2B 
